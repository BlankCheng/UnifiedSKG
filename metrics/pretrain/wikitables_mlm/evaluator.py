...
class EvaluateTool(object):
    ...
    def evaluate(self, preds, golds, section):
        ...
        for pred, gold_result in zip(preds, gold_inferreds):
            ex_match.append(eval_ex_match(pred, gold_result))  # generation exact match # TODO: string may not match even when tokens match, e.g., "pie<mask>" and "pie <mask>"
            # ex_match.append(sorted(pred.split(', ')) == sorted(gold_result.split(', ')))

        summary["all_ex"] = float(np.mean(ex_match))
        summary["avr"] = float(np.mean(ex_match))
        return summary
